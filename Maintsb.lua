...
║                                                                          ║
║   Supports :  Delta Mobile / iOS / iPad / PC / All Executors             ║
║                                                                          ║
║   ─────────────────────────────────────────────────────────────────    ║
║   COMBAT ENGINE                                                          ║
║   · Follow Target  —  HEAD mode  : stick to head, spam skills            ║
║   · Follow Target  —  ORBIT mode : circle target fast, hard to hit       ║
║   · Follow Target  —  FEET mode  : stay at feet, low-profile             ║
║   · Solo Hacker    —  follow all players simultaneously                  ║
║   · Drag Hold      —  pull targets to your position                      ║
║   · Cam Lock       —  camera always locked onto target                   ║
║   · Anti Void      —  rescue if Y drops below threshold                  ║
║   · Stop All       —  kill all combat features instantly                 ║
║                                                                          ║
║   MOVEMENT                                                               ║
║   · Speed Boost (TP-walk, adjustable)                                    ║
║   · Jump Boost (height slider)                                           ║
║   · Gravity slider                                                       ║
║   · FOV slider                                                           ║
║   · Walkspeed slider (direct Humanoid.WalkSpeed)                         ║
║                                                                          ║
║   EXPLOITS (TSB-specific attributes)                                     ║
║   · No Dash Cooldown                                                     ║
║   · No Fatigue                                                           ║
║   · Extra Emote Slots                                                    ║
║   · Emote Search Bar                                                     ║
...
-- ============================================================================
-- [2]  CHARACTER MANAGEMENT  (auto re-link on respawn)
-- ============================================================================
local char, hum, hrp
...
-- ============================================================================
-- [3]  TSB ATTRIBUTE INIT  (workspace/player attributes TSB reads)
-- ============================================================================
local function initTSBAttributes()
    ...
    pcall(function()
        if LocalPlayer:GetAttribute("ExtraSlots")    == nil then
            LocalPlayer:SetAttribute("ExtraSlots",    false) end
        if LocalPlayer:GetAttribute("EmoteSearchBar") == nil then
            LocalPlayer:SetAttribute("EmoteSearchBar", false) end
        if workspace:GetAttribute("NoDashCooldown")  == nil then
            workspace:SetAttribute("NoDashCooldown",   false) end
        if workspace:GetAttribute("NoFatigue")        == nil then
            workspace:SetAttribute("NoFatigue",         false) end
    end)
end
...
local State = {
    ...
    camLock       = false,
    antiVoid      = false,
    followMode    = "HEAD",    -- "HEAD" | "ORBIT" | "FEET"
    studDist      = 3,
    followSpeed   = 20,
    camDist       = 12,
...
local _orbitAngle = 0   -- accumulated orbit angle for ORBIT mode
...
-- ============================================================================
-- [6]  COMBAT ENGINE  (RunService.Heartbeat)
-- ============================================================================
RunService.Heartbeat:Connect(function(dt)
    if not hrp or not char then return end

    -- ── Speed boost (TP-walk direction) ──────────────────────────────────
    if State.speedOn and hum then
        local dir = hum.MoveDirection
        if dir.Magnitude > 0 then
            pcall(function()
                hrp.CFrame = hrp.CFrame + (dir * State.speedVal)
            end)
        end
    end
    ...
    elseif State.followOn and State.target then
        ...
        if tHRP then
            table.insert(targets, {player = State.target, hrp = tHRP})
        end
    end
    ...
    if #targets > 0 then
        ...
        if State.followMode == "ORBIT" then
            -- Smooth orbit: accumulate angle, produce circular position
        ...
        elseif State.followMode == "HEAD" then
            -- Stick to face of target at head height
        ...
        elseif State.followMode == "FEET" then
            -- Stay at foot level of target
            local look  = tHRP.CFrame.LookVector.Unit
            local feetY = tPos.Y - 2.5
            newCF = CFrame.lookAt(
                Vector3.new(tPos.X + look.X * dist, feetY, tPos.Z + look.Z * dist),
                Vector3.new(tPos.X, feetY, tPos.Z)
            )
        end
...
-- ============================================================================
-- [7]  UI THEME  (color palette + dimension system)
-- ============================================================================
local T = {
    -- Window layers
...
-- ============================================================================
pcall(function()
    game.CoreGui:FindFirstChild("__A9xTSBStrong"):Destroy()
end)
...
local wX  = math.floor((vp.X - D.winW) / 2)
local wY  = math.floor((vp.Y - D.winH) / 2)
wX = math.max(0, wX)
wY = math.max(0, wY)
...
winBorder.Transparency = 0
winBorder.Parent      = win

-- ============================================================================
-- [10]  DRAG SYSTEM  (touch + mouse, title bar only)
-- ============================================================================
do
    local dragging, touchId, startInput, startPanelPos = false, nil, nil, nil

    local function isInBtnZone(px)
        local abs = win.AbsolutePosition
        local sz  = win.AbsoluteSize
        return px > abs.X + sz.X - 90  -- right 90px reserved for buttons
    end

    -- We wire drag on titleBar after it's created (see below)
    -- Stored here for reference:
    root._dragSetup = function(titleBar)
...
-- Patch: cover bottom corners of titlebar
...
-- Title icon (colored square as logo placeholder)
...
-- Wire drag
root._dragSetup(titleBar)

-- Minimize logic
local isMinimized = false
local bodyFrame   -- forward declared, set below
...
minBtn.MouseButton1Click:Connect(function()
    ...
    TweenService:Create(win,
        ...
        {Size = UDim2.fromOffset(D.winW, targetH)}
    ):Play()
...
-- ============================================================================
-- [14]  PLAYER PROFILE SECTION  (top of sidebar)
-- ============================================================================
...
task.spawn(function()
    pcall(function()
        ...
        local img, _    = Players:GetUserThumbnailAsync(
            LocalPlayer.UserId, thumbType, thumbSize
        )
        avatarImg.Image = img
    end)
end)
        ...
        if btn then
            if def.id == id then
                ...
                if btn:FindFirstChild("Accent") then
                    btn.Accent.BackgroundTransparency = 0
                end
            else
                btn.BackgroundColor3 = T.tabInactive
                btn.TextColor3       = T.textSec
                if btn:FindFirstChild("Accent") then
                    btn.Accent.BackgroundTransparency = 1
                end
            end
        end
        if panel then
            panel.Visible = (def.id == id)
        end
    end
end
...
-- ============================================================================
-- [16]  SOCIAL LINKS (bottom of sidebar)
-- ============================================================================
...
webBtn.MouseButton1Click:Connect(function()
    -- Open URL via setclipboard + notify (direct URL open not possible in executor)
    pcall(function()
        setclipboard("https://anonymous9x-site.pages.dev/")
    end)
    -- Also try via StarterGui notification
    pcall(function()
...
ytBtn.MouseButton1Click:Connect(function()
    pcall(function()
        setclipboard("https://youtube.com/@anonymous9xch?si=1HClEtCjJCqUjoXj")
    end)
    pcall(function()
        game:GetService("StarterGui"):SetCore("SendNotification", {
            Title    = "Anonymous9x",
...
-- Content header (shows active tab name)
...
-- Update header label when tab changes (wrap setActiveTab)
...
-- ============================================================================
-- [18]  UI COMPONENT LIBRARY  (inline, minimal)
-- ============================================================================

-- Create a scrolling content panel for a tab
...
local function makeToggle(parent, opts)
    -- opts: { title, subtitle, default, order, callback }
    ...
    knob.Position         = opts.default
        and UDim2.fromOffset(trackW - knobSize - 2, 2)
    ...
    local function setToggle(v)
        ...
        TweenService:Create(knob,
            TweenInfo.new(0.14, Enum.EasingStyle.Quad),
            {Position = UDim2.fromOffset(kx, 2)}
        ):Play()
        if opts.callback then opts.callback(v) end
    end

    -- Click hitbox (full card)
...
local function makeSlider(parent, opts)
    -- opts: { title, min, max, default, suffix, step, order, callback }
    ...
    hitbox.TouchPan:Connect(function(_, positions, _)
        if dragging and positions[1] then
            updateFromAbsX(positions[1].X)
        end
    end)
...
local function makeDropdown(parent, opts)
    -- opts: { title, options, default, order, callback }
    local h = 40
    ...
    dropList.Position           = UDim2.new(selBtn.Position.X.Scale, selBtn.Position.X.Offset,
                                             1, 2)
    ...
    local function openDropdown()
        -- Recalculate size of selBtn after layout
    ...
    for i, opt in ipairs(opts.options) do
        ...
        item.MouseEnter:Connect(function()
            item.BackgroundColor3 = T.dropHover
        end)
...
local function makeButton(parent, opts)
    -- opts: { title, subtitle, color, order, callback }
    ...
    btn.MouseButton1Click:Connect(function()
        TweenService:Create(btn,
            TweenInfo.new(0.06),
            {BackgroundColor3 = opts.color and opts.color:Lerp(T.white, 0.15) or T.cardHover}
        ):Play()
        task.delay(0.12, function()
            TweenService:Create(btn,
                TweenInfo.new(0.10),
                {BackgroundColor3 = opts.color or T.btnNeutral}
            ):Play()
        end)
    ...
    btn.MouseEnter:Connect(function()
        TweenService:Create(btn,
            TweenInfo.new(0.10),
            {BackgroundColor3 = (opts.color or T.btnNeutral):Lerp(T.white, 0.08)}
        ):Play()
    end)
...
-- Target dropdown (live list updated each time tab becomes visible)
local ddTargetCard, getTarget do
    ddTargetCard, getTarget = makeDropdown(combatPanel, {
        title    = "Select Target",
        ...
        callback = function(val)
            if val == "— None —" then
                ...
                State.target = Players:FindFirstChild(val)
            end
        end,
    })
end
...
makeDropdown(combatPanel, {
    ...
    callback = function(val)
        ...
        _orbitAngle = 0  -- reset orbit when changing mode
    end,
})

makeSlider(combatPanel, {
    title    = "Stud Distance",
    min      = 1,
    max      = 20,
    default  = 3,
    ...
    step     = 0.5,
    order    = co(),
    callback = function(val) State.studDist = val end,
})
...
makeSlider(combatPanel, {
    ...
    step     = 1,
    order    = co(),
    callback = function(val) State.followSpeed = val end,
})
...
makeToggle(combatPanel, {
    title    = "Follow Target",
    subtitle = "Stick to selected target",
    default  = false,
    order    = co(),
    callback = function(val)
        State.followOn = val
...
makeToggle(combatPanel, {
    title    = "Solo Hacker",
    subtitle = "Follow all players simultaneously",
    default  = false,
    order    = co(),
    callback = function(val) State.soloHacker = val end,
})

makeToggle(combatPanel, {
    title    = "Drag Hold",
    subtitle = "Pull targets to your position",
    default  = false,
    order    = co(),
    callback = function(val) State.dragHold = val end,
})

makeToggle(combatPanel, {
    title    = "Cam Lock",
    subtitle = "Camera locks onto target",
    default  = false,
    order    = co(),
    callback = function(val)
        State.camLock = val
...
makeToggle(combatPanel, {
    title    = "Anti Void",
    subtitle = "Rescue if character falls below -250Y",
    default  = false,
    order    = co(),
    callback = function(val) State.antiVoid = val end,
})

makeDivider(combatPanel, co())

makeButton(combatPanel, {
    title    = "Stop All Combat",
    subtitle = "Disable all follow / lock features",
    color    = T.btnDanger,
    order    = co(),
    callback = function()
        State.followOn   = false
        State.soloHacker = false
...
makeButton(combatPanel, {
    ...
    callback = function()
        ...
        if tH and hrp then
            pcall(function()
                hrp.CFrame = tH.CFrame + Vector3.new(3, 0, 0)
            end)
        end
    end,
})
...
makeToggle(movPanel, {
    title    = "Speed Boost",
    subtitle = "TP-walk in move direction",
    default  = false,
    order    = mo(),
    callback = function(val) State.speedOn = val end,
})
...
makeSlider(movPanel, {
    ...
    step     = 0.1,
    order    = mo(),
    callback = function(val) State.speedVal = val end,
})

makeToggle(movPanel, {
    title    = "Walkspeed Lock",
    subtitle = "Force Humanoid.WalkSpeed",
    default  = false,
    order    = mo(),
    callback = function(val)
        State.wsOn = val
        if not val and hum then
...
makeToggle(movPanel, {
    title    = "Jump Boost",
    subtitle = "Override jump height",
    default  = false,
    order    = mo(),
    callback = function(val)
        State.jumpOn = val
        if hum then
            pcall(function() hum.UseJumpPower = not val end)
        end
    end,
})
...
makeToggle(expPanel, {
    ...
    subtitle = "workspace NoDashCooldown = true",
    default  = false,
    order    = eo(),
    callback = function(val)
        State.noDashCD = val
        pcall(function() workspace:SetAttribute("NoDashCooldown", val) end)
    end,
})

makeToggle(expPanel, {
    title    = "No Fatigue",
    subtitle = "workspace NoFatigue = true",
    default  = false,
    order    = eo(),
    callback = function(val)
        State.noFatigue = val
        pcall(function() workspace:SetAttribute("NoFatigue", val) end)
    end,
})
...
makeToggle(expPanel, {
    ...
    subtitle = "LocalPlayer ExtraSlots = true",
    default  = false,
    order    = eo(),
    callback = function(val)
        State.extraSlots = val
        pcall(function() LocalPlayer:SetAttribute("ExtraSlots", val) end)
    end,
})

makeToggle(expPanel, {
    title    = "Emote Search Bar",
    subtitle = "LocalPlayer EmoteSearchBar = true",
    default  = false,
    order    = eo(),
    callback = function(val)
        State.emoteSearch = val
        pcall(function() LocalPlayer:SetAttribute("EmoteSearchBar", val) end)
    end,
})
...
makeButton(expPanel, {
    title    = "Spoof VIP Server Owner",
    subtitle = "Sets VIPServer attribute to your UID",
    color    = T.btnNeutral,
    order    = eo(),
    callback = function()
        pcall(function()
...
makeButton(tpPanel, {
    title    = "Teleport to Target",
    subtitle = "Select target in Combat tab first",
    color    = T.btnPrimary,
    order    = tto(),
    callback = function()
        if not State.target then return end
...
-- ============================================================================
-- [24]  BOOT — set initial tab, load profile
-- ============================================================================
setActiveTab("Combat")

-- Periodically update DisplayName / username (they can change)
task.spawn(function()
    task.wait(2)
    pcall(function()
        displayNameLbl.Text = LocalPlayer.DisplayName
...
-- ============================================================================
-- END OF Anonymous9x TSB Strong v1.0
-- https://anonymous9x-site.pages.dev/
-- ============================================================================
